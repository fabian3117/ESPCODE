...
void sendEnableToLPC(AsyncWebServerRequest *request){
  Serial.println(request.getAll);
  
  String introduccionClave="<input name='val'placeholder='Introduci clave'type='number' />";
  String btnConfirmacion="<input type='submit' value='Subir'/>";

  
Serial.println("ENTRO A ACTIVACION");
  request->send(200, "text/html", introduccionClave+btnConfirmacion);
}
